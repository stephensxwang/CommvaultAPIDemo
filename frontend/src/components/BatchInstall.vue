  ...
  <el-container>
    <el-header style="text-align: left;">
      <el-upload
        ...
        <el-button size="small" @click="handleSubmit" type="primary">Submit Installation</el-button>
        <el-link type="info" href="/file/clientinstall.xlsx">Installation Template</el-link>
        <div class="el-upload__tip" slot="tip">excel file with server info</div>
      </el-upload>
    </el-header>
    <el-main>
      <el-table highlight-current-row :data="tableData" empty-text="Please upload the server info" style="top: 20px;width: 100%">
    ...
    handleSubmit() {
      if(this.tableData && this.tableData.length == 0){
        this.$message({
          type: 'error',
          duration: 5000,
          message: "Please upload the server info first",
          showClose: true
        });
        return;
      }
      ...
      .then(function (response) {
        ...
        if(response && response.data){
          ...
          that.$message({
            type: 'success',
            message: 'Installation Job Submitted'
          });
          that.tableData = view.contentList;
        }
      })
      .catch(function (error) {
        that.$message({
